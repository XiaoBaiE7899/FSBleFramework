...
#import "FSLibHelp.h"
// !!!: SDK 嵌套在demo工程里，只要SDK的源码有任何修改，都要重新编译，否则在demo项目里面不会生效
...
/*
  ...
  02 50 02 52 03   速度参数
  02 50 02 5A 05 01 0C 03

  02 50 03 53 03   坡度参数
  02 50 03 0F 00 03 5F 03

  02 50 04 54 03   累计里程
  02 50 04 69 00 00 00 3D 03

  02 51 51 03
  02 51 00 51 03

  02 53 00 00 00 00 00 46 AA 19 00 A6 03   写入用户信息
  02 53 00 53 03

  02 53 01 00 00 00 00 80 00 00 00 D2 03   准备开始
  02 53 01 03 51 03

  02 53 00 00 00 00 00 46 AA 19 00 A6 03   写入用户数据
  02 53 00 53 03
  ...
  跑客跑步机 迈动生产，有防止空炮，支持暂停，不支持指令恢复，没有下发配置信息
  1连接成功 速度参数、坡度参数、累计里程、写入用户信息
  2启动过程 准备开始（53 01）、写入用户数据（53 00）
  3暂停 发送的是停止指令(53 03)
  4恢复 发送的是启动指令（53 01）无法启动

  02 50 02 52 03
  02 50 02 50 0A 00 08 03

  02 50 03 53 03
  02 50 03 16 00 45 03

  02 50 04 54 03
  02 50 04 0E 04 00 00 5E 03

  02 51 51 03
  02 51 00 51 03

  02 53 00 00 00 00 00 46 AA 19 00 A6 03
  02 53 00 00 00 00 00 53 03

  02 53 01 00 00 00 00 00 00 00 00 52 03
  02 53 01 03 00 51 03

  02 53 00 00 00 00 00 46 AA 19 00 A6 03
  02 53 00 00 00 00 00 53 03

  02 53 03 50 03  第一条停止指令发送以后，设备进入暂停状态 设备上显示PAU 这个是没有断连， 设备状态为暂停
  02 53 03 50 03

  02 53 03 50 03  第二条停止指令发送以后，设备完全停止，设置状态为正常待机桩体
  02 53 03 50 03
 */
