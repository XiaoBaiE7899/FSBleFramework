    ...
    if (_commands.count && ++_cntError <= cmd.sendCnt) { // 发送指令  错误次数没超过
        ...
        if (_cntError > 1) {
            ...
            if (self.module.isFitshow &&
                ...
                [[FSBleTools dataToString:cmd.data] hasPrefix:@"02 53 00"]
...
//            FSLog(@"发送(%@): %@", _module.name, cmd.data.fstoString());
            FSLog(@"发送(%@): %@", _module.name, [FSBleTools dataToString:cmd.data]);
...
//    FSLog(@"接收(%@): %@", peripheral.name, characteristic.value.fstoString());
    FSLog(@"接收(%@): %@", _module.name, [FSBleTools dataToString:characteristic.value]);
    _receiveCmd.chrt = characteristic;
    _receiveCmd.data = characteristic.value;
    // MARK: 20220111  健腹轮  上报的数据可能为空
    if (kFSIsEmptyString([FSBleTools dataToString:characteristic.value])) {
        return;
    }
